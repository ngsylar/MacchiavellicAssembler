...
Simbolo auxiliar; //pega uma linha, le palavra a palavra
...
void onepass (string* file_name) {
    ...
    int qtdd;
    string labelzinho[2];
    int i_labelzinho, labelzinho_eh_vetor;
    line_number++;
    ...
    while ( !tokenizer.eof() ) {
        ...
        switch ( OPCODE[token] ) {
            ...
            case COPY:      
                ...
                if (!tokenizer.eof() && tokenizer>>token) {
                    i_labelzinho = 0;
                    labelzinho_eh_vetor = 0;
                    for (int i=0; i < token.size(); i++) {
                        if (token[i] != ',') {
                            labelzinho[i_labelzinho].push_back(token[i]);
                            labelzinho_eh_vetor = 1;
                        } else {
                            if (i_labelzinho ==  0) {
                                i_labelzinho = 1;
                            } else {
                                std::cout << endl << "Line " << line_number << " of [" << *file_name << "]:" << endl;
                                std::cout << "syntactic error: too many arguments" << endl;
                                break;
                            }
                        }
                    }
                    // testa se labelzinho ja esta definido
                    // nota: testa labelzinho[0] ja esta definido, se sim push endereco, se nao push -1
                    word.check_label(file_name, labelzinho[0], 0); // dentro do se nao abaixo de push -1
                    if (labelzinho_eh_vetor) {
                        word.check_label(file_name, labelzinho[1], 0);
                    } else {
                        // nota: mostrar erro faltando argumentos
                    }
                } else {
                // nota: mostrar erro faltando argumentos
                }
                    ...
                    case d_SPACE:
                        ...
                        if (tokenizer.eof() && tokenizer>>token) { 
                            if (word.check_const(file_name, token)) {//token eh um numero
                                ...
                                while (tokenizer.eof() && tokenizer>>token) {
                                    std::cout << endl << "Line " << line_number << " of [" << *file_name << "]:" << endl;
                                    std::cout << "syntactic error: too many arguments" << endl;
                                }
                            }
                        ...
                        for(i=0; i<tabelasimbolos.size(); i++){    // vai varrer o vector da tabela de simbolos buscando o token (nao eh endereco ou instrucao)
                            ...
                            if (tabelasimbolos[i].simbolo == label.content()){        //encontrou
                                ...
                                if(tabelasimbolos[i].definido == true) { //se ja estava definido
                                    depoispass[auxcont] = tabelasimbolos[i].valor; //se esse simbolo encontrado ja for a definicao, coloca esse endereco no arquivo de saida
                                }

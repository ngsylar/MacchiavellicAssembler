    ...
    while (tokenizer >> token) {    // enquanto linha nao acabou, pega um token
        ...
        switch ( DIRECTIVE[ token ] ) {
            ...
            case d_IF:
                // se linha nao acabou, pega proximo token
                // if (!tokenizer.eof() && (tokenizer >> token)) {
                //     // pega um label (ja inserido na tabela) ou constante
                //     if ((token.front() >= 48) && (token.front() <= 57)) {
                //         word.check_const (file_name, token);
                //     }
                //     //ident_table.search (token);
                // } else // erro
